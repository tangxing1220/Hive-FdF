...
{
	if (key == MLX_NUM_PAD_PLUS || key == MLX_M_SCROLL_UP)
		redraw->ctler->camera_zoom++;
	else if (key == MLX_NUM_PAD_MINUS || key == MLX_M_SCROLL_DOWN)
		redraw->ctler->camera_zoom--;
...
void	image_projection(int key, t_redraw *redraw)
{
	...
	if (key == MLX_MAIN_PAD_I)
		redraw->ctler->projection_type = PRO_ISO;
	else if (key == MLX_MAIN_PAD_P)
		redraw->ctler->projection_type = PRO_PARALLEL;
	draw_image_blackground(redraw->fdf);
	draw_image(redraw->image_map, redraw->fdf, redraw->ctler);
	mlx_put_image_to_window(redraw->fdf->mlx_ptr,\
							redraw->fdf->win_ptr, redraw->fdf->img_ptr, 0, 0);
